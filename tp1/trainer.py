...
class EarlyStopping():
	...
	def __call__(self, model:torch.nn.Module, val_loss: float):
		"""
		Executes logic when calling EarlyStopping object e.g
...
class Trainer():
	...
	def __init__(
		...
		self.scaler = None  # Placeholder for AMP scaler if needed
	...
	def train_model_v2(self, use_amp: bool = False, dtype: torch.dtype = torch.bfloat16):
		"""
			...
			"""
		model = self.model.train()
		scaler = torch.amp.grad_scaler.GradScaler(device=self.device,enabled=use_amp)
		losses = []
