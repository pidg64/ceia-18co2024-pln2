...
def populate_vector_database():
    """
    Populates the vector database with resumes from the 'cvs' directory.
    """
    ...
    if not os.path.isdir(cvs_dir):
        print(f"Error: Directory '{cvs_dir}' not found.")
        return
    for filename in os.listdir(cvs_dir):
        if filename.endswith('.txt'):
            resume_path = os.path.join(cvs_dir, filename)            
            # Generate index name from filename
            base_name = filename.removesuffix('_resume.txt').replace('_', '-')
            ...
            try:
                ...
                chunks = parse_resume_to_chunks(
                    resume_text, chunk_size=625, overlap=125
                )                
                if not chunks:
